...
#define LOCTEXT_NAMESPACE "SBangoScriptClassViewer"

//////////////////////////////////////////////////////////////

class FUnloadedBangoScriptBlueprintData : public IUnloadedBlueprintData
{
public:
	FUnloadedBangoScriptBlueprintData(TWeakPtr<FBangoScriptClassViewerNode> InClassViewerNode);

	virtual ~FUnloadedBangoScriptBlueprintData() {}
	...
	const TWeakPtr<FBangoScriptClassViewerNode>& GetClassViewerNode() const;
...
private:
	...
	TWeakPtr<FBangoScriptClassViewerNode> ClassViewerNode;
};


struct FBangoScriptClassViewerNodeNameLess
{
	FBangoScriptClassViewerNodeNameLess(EClassViewerNameTypeToDisplay NameTypeToDisplay = EClassViewerNameTypeToDisplay::ClassName) : NameTypeToDisplay(NameTypeToDisplay) {}

	bool operator()(TSharedPtr<FBangoScriptClassViewerNode> A, TSharedPtr<FBangoScriptClassViewerNode> B) const
	{ 
		check(A.IsValid());
		check(B.IsValid());
...
public:
	 ...
	 */	
	void SortChildren(TSharedPtr< FBangoScriptClassViewerNode >& InRootNode);

	/** Checks if a particular class is placeable.
	 *	@return The ObjectClassRoot for building a duplicate tree using.
	 */
	const TSharedPtr< FBangoScriptClassViewerNode > GetObjectRootNode() const
	{
	 ...
	 */
	TSharedPtr< FBangoScriptClassViewerNode > FindParent(const TSharedPtr< FBangoScriptClassViewerNode >& InRootNode, FTopLevelAssetPath InParentClassname, const UClass* InParentClass);
	...
	*/
	 ...
	 *	@return The node.
	 */
	TSharedPtr< FBangoScriptClassViewerNode > FindNodeByGeneratedClassPath(const TSharedPtr< FBangoScriptClassViewerNode >& InRootNode, FTopLevelAssetPath InGeneratedClassPath);
...
private:
	 ...
	 */	
	void CreateNodesForLoadedClasses( TSharedPtr<FBangoScriptClassViewerNode>& OutRootNode, TMap<FTopLevelAssetPath, TSharedPtr< FBangoScriptClassViewerNode >>& InOutClassPathToNode );
	 ...
	 */
	void CreateOrUpdateUnloadedClassNode(TSharedPtr<FBangoScriptClassViewerNode>& InOutClassViewerNode, const FAssetData& InAssetData, FTopLevelAssetPath InClassPath);
	 ...
	 */
	void FindClass(TSharedPtr< FBangoScriptClassViewerNode > InOutClassNode);

	/** Sets the fields calculated from the in-memory UClass on the given Node, if not already set. */
	void SetClassFields(TSharedPtr<FBangoScriptClassViewerNode>& InOutClassNode, UClass& Class);
	 ...
	 */
	bool FindAndRemoveNodeByClassPath(const TSharedPtr< FBangoScriptClassViewerNode >& InRootNode, FTopLevelAssetPath InClassPath);
...
private:
	...
	TSharedPtr< FBangoScriptClassViewerNode > ObjectClassRoot;
...
namespace ClassViewer
{
	namespace Helpers
	{
			...
			const TSharedPtr<FBangoScriptsScriptContainerClassViewerFilter>& InClassFilter, const FClassViewerInitializationOptions& InInitOptions)
		{
			...
			{
				TSharedPtr< FBangoScriptClassViewerNode > NewNode = MakeShared<FBangoScriptClassViewerNode>( *ChildList[ChildIdx].Get() );
			...
			{
				InOutRootNode->GetChildrenList().Sort(FBangoScriptClassViewerNodeNameLess(InInitOptions.NameTypeToDisplay));
			}
		 ...
		 */
		static void GetClassTree(TSharedPtr< FBangoScriptClassViewerNode >& InOutRootNode, const TSharedPtr<FBangoScriptsScriptContainerClassViewerFilter>& InClassFilter, const FClassViewerInitializationOptions& InInitOptions)
		{
			const TSharedPtr< FBangoScriptClassViewerNode > ObjectClassRoot = ClassHierarchy->GetObjectRootNode();

			// Duplicate the node, it will have no children.
			InOutRootNode = MakeShared<FBangoScriptClassViewerNode>(*ObjectClassRoot);

			if (InInitOptions.bIsActorsOnly)
			{
				for (int32 ClassIdx = 0; ClassIdx < ObjectClassRoot->GetChildrenList().Num(); ClassIdx++)
				{
					TSharedPtr<FBangoScriptClassViewerNode> ChildNode = MakeShared<FBangoScriptClassViewerNode>(*ObjectClassRoot->GetChildrenList()[ClassIdx].Get());
			...
			const TSharedPtr< FBangoScriptsScriptContainerClassViewerFilter >& InClassFilter, const FClassViewerInitializationOptions& InInitOptions)
		{
			...
			for(const TSharedPtr<FBangoScriptClassViewerNode>& ChildNode : InOriginalRootNode->GetChildrenList())
			{
			...
			const FClassViewerInitializationOptions& InInitOptions)
		{
			...
			for(const TSharedPtr<FBangoScriptClassViewerNode>& ChildNode : ObjectClassRoot->GetChildrenList())
			{
				AddChildren_List(InOutNodeList, ChildNode, InClassFilter, InInitOptions);
			}
		}
		 ...
		 */
		static void LoadClass(TSharedPtr< FBangoScriptClassViewerNode > InOutClassNode)
		{
...
DECLARE_DELEGATE_OneParam( FOnClassItemDoubleClickDelegate, TSharedPtr<FBangoScriptClassViewerNode> );
...
public:
	...
	SLATE_BEGIN_ARGS( SClassItem )
		...
		SLATE_ARGUMENT( TSharedPtr<FBangoScriptClassViewerNode>, AssociatedNode)
	...
	void Construct( const FArguments& InArgs, const TSharedRef<STableViewBase>& InOwnerTableView )
	{
		...
		struct Local
		{
			...
			static TSharedPtr<SToolTip> GetToolTip(TSharedPtr<FBangoScriptClassViewerNode> AssociatedNode)
			{
...
private:
	...
	TSharedPtr< FBangoScriptClassViewerNode > AssociatedNode;
...
void FClassHierarchy::CreateNodesForLoadedClasses(TSharedPtr<FBangoScriptClassViewerNode>& OutRootNode, TMap<FTopLevelAssetPath, TSharedPtr< FBangoScriptClassViewerNode >>& InOutClassPathToNode)
{
	for (TObjectIterator<UClass> ClassIt; ClassIt; ++ClassIt)
	{
		...
		TSharedPtr<FBangoScriptClassViewerNode>& Node = InOutClassPathToNode.FindOrAdd(CurrentClass->GetClassPathName());
		if (!Node)
		{
			Node = MakeShared<FBangoScriptClassViewerNode>(CurrentClass->GetName(), CurrentClass->GetDisplayNameText().ToString());
		}
		SetClassFields(Node, *CurrentClass);
	}
	TSharedPtr<FBangoScriptClassViewerNode>* ExistingRoot = InOutClassPathToNode.Find(UObject::StaticClass()->GetClassPathName());
	check(ExistingRoot && ExistingRoot->IsValid());
	OutRootNode = *ExistingRoot;
}

TSharedPtr< FBangoScriptClassViewerNode > FClassHierarchy::FindParent(const TSharedPtr< FBangoScriptClassViewerNode >& InRootNode, FTopLevelAssetPath InParentClassname, const UClass* InParentClass)
{
	...
	TSharedPtr< FBangoScriptClassViewerNode > ReturnNode;
...
TSharedPtr< FBangoScriptClassViewerNode > FClassHierarchy::FindNodeByGeneratedClassPath(const TSharedPtr< FBangoScriptClassViewerNode >& InRootNode, FTopLevelAssetPath InGeneratedClassPath)
{
...
{
	TSharedPtr< FBangoScriptClassViewerNode > Node = FindNodeByGeneratedClassPath(ObjectClassRoot, InGeneratedClassPath);
...
bool FClassHierarchy::FindAndRemoveNodeByClassPath(const TSharedPtr< FBangoScriptClassViewerNode >& InRootNode, FTopLevelAssetPath InClassPath)
{
	FixupClassCoreRedirects(InClassPath);

	TFunction<bool(const TSharedPtr<FBangoScriptClassViewerNode>&)> FindAndRemoveNodeRecursive;
	FindAndRemoveNodeRecursive = [&InClassPath, &FindAndRemoveNodeRecursive](const TSharedPtr<FBangoScriptClassViewerNode>& InRootNode)
		{
			// Search the children recursively, one of them might have the parent.
			TArray<TSharedPtr<FBangoScriptClassViewerNode>>& ChildrenList = InRootNode->GetChildrenList();
...
void FClassHierarchy::AddAsset(const FAssetData& InAddedAssetData)
{
	...
	{
		TSharedPtr<FBangoScriptClassViewerNode> NewNode;
		...
		TSharedPtr< FBangoScriptClassViewerNode > ParentNode = FindParent(ObjectClassRoot, NewNode->ParentClassPath, ParentClass); 
...
void FClassHierarchy::SortChildren( TSharedPtr< FBangoScriptClassViewerNode >& InRootNode)
{
	TArray< TSharedPtr< FBangoScriptClassViewerNode > >& ChildList = InRootNode->GetChildrenList();
	...
	// Sort the children.
	ChildList.Sort(FBangoScriptClassViewerNodeNameLess());
}

void FClassHierarchy::FindClass(TSharedPtr<FBangoScriptClassViewerNode> InOutClassNode)
{
...
void FClassHierarchy::SetClassFields(TSharedPtr<FBangoScriptClassViewerNode>& InOutClassNode, UClass& Class)
{
...
void FClassHierarchy::CreateOrUpdateUnloadedClassNode(TSharedPtr<FBangoScriptClassViewerNode>& InOutClassViewerNode, const FAssetData& InAssetData, FTopLevelAssetPath InClassPath)
{
	if (!InOutClassViewerNode)
	{
		...
		InOutClassViewerNode = MakeShared<FBangoScriptClassViewerNode>(ClassName, ClassDisplayName);
	}
	...
	TSharedPtr<FUnloadedBangoScriptBlueprintData> UnloadedBlueprintData = MakeShareable(new FUnloadedBangoScriptBlueprintData(InOutClassViewerNode));
...
void FClassHierarchy::PopulateClassHierarchy()
{
	...
	TMap<FTopLevelAssetPath, TSharedPtr<FBangoScriptClassViewerNode>> ClassPathToNode;
	...
	// Retrieve all blueprint classes 
	{
		...
		for (const FAssetData& AssetData : Assets)
		{
			...
			if (!ClassPath.IsNull())
			{
				...
				TSharedPtr<FBangoScriptClassViewerNode>& Node = ClassPathToNode.FindOrAdd(ClassPath);
		...
		for (const FAssetData& AssetData : Assets)
		{
			...
			TSharedPtr<FBangoScriptClassViewerNode>& Node = ClassPathToNode.FindOrAdd(ClassPathNameFromAssetPath);
	...
	for (TPair<FTopLevelAssetPath, TSharedPtr<FBangoScriptClassViewerNode>>& KVPair : ClassPathToNode)
	{
		TSharedPtr<FBangoScriptClassViewerNode>& Node = KVPair.Value;
		if (Node == ObjectClassRoot)
		{
			// No parent expected for the root class
			continue;
		}
		TSharedPtr<FBangoScriptClassViewerNode>* ParentNodePtr = nullptr;
		...
		TSharedPtr<FBangoScriptClassViewerNode>& ParentNode = *ParentNodePtr;
		check(ParentNode);
...
void SBangoScriptClassViewer::Construct(const FArguments& InArgs, const FClassViewerInitializationOptions& InInitOptions )
{
	bNeedsRefresh = true;
	NumClasses = 0;
	
	// Listen for when view settings are changed
	UClassViewerSettings::OnSettingChanged().AddSP(this, &SBangoScriptClassViewer::HandleSettingChanged);
	...
	ClassFilter = MakeShared<FBangoScriptsScriptContainerClassViewerFilter>(InInitOptions);
	...
	if(InitOptions.Mode == EClassViewerMode::ClassBrowsing)
	{
		...
		.OnGetMenuContent(this, &SBangoScriptClassViewer::FillFilterEntries)
		.HasDownArrow(true)
	...
	if ( InitOptions.Mode == EClassViewerMode::ClassBrowsing )
	{
		OnContextMenuOpening = FOnContextMenuOpening::CreateSP(this, &SBangoScriptClassViewer::BuildMenuWidget);
	}

	SAssignNew(ClassList, SListView<TSharedPtr< FBangoScriptClassViewerNode > >)
		.SelectionMode(ESelectionMode::Single)
		.ListItemsSource(&RootTreeItems)
		// Generates the actual widget for a tree item
		.OnGenerateRow(this, &SBangoScriptClassViewer::OnGenerateRowForClassViewer)
		// Generates the right click menu.
		.OnContextMenuOpening(OnContextMenuOpening)
		// Find out when the user selects something in the tree
		.OnSelectionChanged(this, &SBangoScriptClassViewer::OnClassViewerSelectionChanged)
		.HeaderRow
		(
			SNew(SHeaderRow)
	...
	SAssignNew(ClassTree, STreeView<TSharedPtr< FBangoScriptClassViewerNode > >)
		.SelectionMode(ESelectionMode::Single)
		.TreeItemsSource(&RootTreeItems)
		// Called to child items for any given parent item
		.OnGetChildren(this, &SBangoScriptClassViewer::OnGetChildrenForClassViewerTree)
		// Called to handle recursively expanding/collapsing items
		.OnSetExpansionRecursive(this, &SBangoScriptClassViewer::SetAllExpansionStates_Helper)
		// Generates the actual widget for a tree item
		.OnGenerateRow(this, &SBangoScriptClassViewer::OnGenerateRowForClassViewer)
		// Generates the right click menu.
		.OnContextMenuOpening(OnContextMenuOpening)
		...
		.OnExpansionChanged(this, &SBangoScriptClassViewer::OnClassViewerExpansionChanged)
		.HeaderRow
		(
			SNew(SHeaderRow)
			.Visibility(EVisibility::Collapsed)
	...
	TSharedRef<STreeView<TSharedPtr< FBangoScriptClassViewerNode > > > ClassTreeView = ClassTree.ToSharedRef();
	TSharedRef<SListView<TSharedPtr< FBangoScriptClassViewerNode > > > ClassListView = ClassList.ToSharedRef();
		...
		[
			...
			[
				...
				[
					...
					.OnTextChanged( this, &SBangoScriptClassViewer::OnFilterTextChanged )
					.OnTextCommitted( this, &SBangoScriptClassViewer::OnFilterTextCommitted )
				]
				...
				[
					...
					.HasDownArrow(false)
					.OnGetMenuContent(this, &SBangoScriptClassViewer::GetViewButtonContent)
					.ButtonContent()
					[
						SNew(SImage)
			...
			[
				SNew(STextBlock)
				.Text(this, &SBangoScriptClassViewer::GetClassCountText)
			]
		]
	];
	...
	if ( InitOptions.Mode == EClassViewerMode::ClassPicker && InitOptions.DisplayMode == EClassViewerDisplayMode::ListView )
	{
		this->ChildSlot
		[
			SNew(SListViewSelectorDropdownMenu<TSharedPtr<FBangoScriptClassViewerNode>>, SearchBox, ClassList)
			[
	...
	ClassViewer::Helpers::PopulateClassviewerDelegate.AddSP(this, &SBangoScriptClassViewer::Refresh);
...
TSharedRef<SWidget> SBangoScriptClassViewer::GetContent()
{
	return SharedThis( this );
}

SBangoScriptClassViewer::~SBangoScriptClassViewer()
{
...
void SBangoScriptClassViewer::ClearSelection()
{
	ClassTree->ClearSelection();
}

void SBangoScriptClassViewer::OnGetChildrenForClassViewerTree( TSharedPtr<FBangoScriptClassViewerNode> InParent, TArray< TSharedPtr< FBangoScriptClassViewerNode > >& OutChildren )
{
	// Simply return the children, it's already setup.
	OutChildren = InParent->GetChildrenList();
}

void SBangoScriptClassViewer::OnClassViewerSelectionChanged( TSharedPtr<FBangoScriptClassViewerNode> Item, ESelectInfo::Type SelectInfo )
{
...
void SBangoScriptClassViewer::OnClassViewerExpansionChanged(TSharedPtr<FBangoScriptClassViewerNode> Item, bool bExpanded)
{
	// Sometimes the item is not valid anymore due to filtering.
	if (Item.IsValid() == false || Item->IsRestricted())
	{
...
TSharedPtr< SWidget > SBangoScriptClassViewer::BuildMenuWidget()
{
	bool bIsBlueprint;
	bool bHasBlueprint;
	TArray< TSharedPtr< FBangoScriptClassViewerNode > > SelectedList;
...
TSharedRef< ITableRow > SBangoScriptClassViewer::OnGenerateRowForClassViewer( TSharedPtr<FBangoScriptClassViewerNode> Item, const TSharedRef< STableViewBase >& OwnerTable )
{	
	...
	TSharedRef< SClassItem > ReturnRow = SNew(SClassItem, OwnerTable)
		...
		.OnDragDetected(this, &SBangoScriptClassViewer::OnDragDetected)
		.OnClassItemDoubleClicked(FOnClassItemDoubleClickDelegate::CreateSP(this, &SBangoScriptClassViewer::ToggleExpansionState_Helper));
...
const TArray< TSharedPtr< FBangoScriptClassViewerNode > > SBangoScriptClassViewer::GetSelectedItems() const
{
...
const int SBangoScriptClassViewer::GetNumItems() const
{
	return NumClasses;
}

TSharedRef<SWidget> SBangoScriptClassViewer::GetViewButtonContent()
{
	// Get all menu extenders for this context menu from the content browser module

	FMenuBuilder MenuBuilder(/*bInShouldCloseWindowAfterMenuSelection=*/true, nullptr, nullptr, /*bCloseSelfOnly=*/ true);

	MenuBuilder.AddMenuEntry(LOCTEXT("ExpandAll", "Expand All"), LOCTEXT("ExpandAll_Tooltip", "Expands the entire tree"), FSlateIcon(), FUIAction(FExecuteAction::CreateSP(this, &SBangoScriptClassViewer::SetAllExpansionStates, bool(true))), NAME_None, EUserInterfaceActionType::Button);
	MenuBuilder.AddMenuEntry(LOCTEXT("CollapseAll", "Collapse All"), LOCTEXT("CollapseAll_Tooltip", "Collapses the entire tree"), FSlateIcon(), FUIAction(FExecuteAction::CreateSP(this, &SBangoScriptClassViewer::SetAllExpansionStates, bool(false))), NAME_None, EUserInterfaceActionType::Button);
	...
	MenuBuilder.BeginSection("Filters", LOCTEXT("ClassViewerFiltersHeading", "Class Filters"));
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SBangoScriptClassViewer::ToggleShowInternalClasses),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &SBangoScriptClassViewer::IsShowingInternalClasses)
				),
			NAME_None,
		...
		for (const TSharedRef<FClassViewerFilterOption>& FilterOption : CustomClassFilterOptions)
		{
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateSP(this, &SBangoScriptClassViewer::ToggleCustomFilterOption, FilterOption),
					FCanExecuteAction(),
					FIsActionChecked::CreateSP(this, &SBangoScriptClassViewer::IsCustomFilterOptionEnabled, FilterOption)
				),
				NAME_None,
				EUserInterfaceActionType::ToggleButton
			);
	...
	MenuBuilder.BeginSection("DeveloperViewType", LOCTEXT("DeveloperViewTypeHeading", "Developer Folder Filter"));
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SBangoScriptClassViewer::SetCurrentDeveloperViewType, EClassViewerDeveloperType::CVDT_None),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &SBangoScriptClassViewer::IsCurrentDeveloperViewType, EClassViewerDeveloperType::CVDT_None)
				),
			NAME_None,
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SBangoScriptClassViewer::SetCurrentDeveloperViewType, EClassViewerDeveloperType::CVDT_CurrentUser),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &SBangoScriptClassViewer::IsCurrentDeveloperViewType, EClassViewerDeveloperType::CVDT_CurrentUser)
				),
			NAME_None,
			EUserInterfaceActionType::RadioButton
			);
		...
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SBangoScriptClassViewer::SetCurrentDeveloperViewType, EClassViewerDeveloperType::CVDT_All),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &SBangoScriptClassViewer::IsCurrentDeveloperViewType, EClassViewerDeveloperType::CVDT_All)
				),
			NAME_None,
			EUserInterfaceActionType::RadioButton
			);
...
void SBangoScriptClassViewer::SetCurrentDeveloperViewType(EClassViewerDeveloperType NewType)
{
...
EClassViewerDeveloperType SBangoScriptClassViewer::GetCurrentDeveloperViewType() const
{
...
bool SBangoScriptClassViewer::IsCurrentDeveloperViewType(EClassViewerDeveloperType ViewType) const
{
	return GetCurrentDeveloperViewType() == ViewType;
}

void SBangoScriptClassViewer::GetInternalOnlyClasses(TArray<FSoftClassPath>& Classes)
{
	if (!InitOptions.bAllowViewOptions)
	{
		return;
...
void SBangoScriptClassViewer::GetInternalOnlyPaths(TArray<FDirectoryPath>& Paths)
{
	if (!InitOptions.bAllowViewOptions)
	{
		return;
...
FText SBangoScriptClassViewer::GetClassCountText() const
{
...
void SBangoScriptClassViewer::ExpandRootNodes()
{
...
FReply SBangoScriptClassViewer::OnDragDetected( const FGeometry& Geometry, const FPointerEvent& PointerEvent )
{
	if(InitOptions.Mode == EClassViewerMode::ClassBrowsing)
	{
		const TArray< TSharedPtr< FBangoScriptClassViewerNode > > SelectedItems = GetSelectedItems();

		if ( SelectedItems.Num() > 0 && SelectedItems[0].IsValid() )
		{
			TSharedRef< FBangoScriptClassViewerNode > Item = SelectedItems[0].ToSharedRef();
...
void SBangoScriptClassViewer::OnOpenBlueprintTool()
{
	ClassViewer::Helpers::OpenBlueprintTool(RightClickBlueprint);
}

void SBangoScriptClassViewer::FindInContentBrowser()
{
	ClassViewer::Helpers::FindInContentBrowser(RightClickBlueprint, RightClickClass);
}

void SBangoScriptClassViewer::OnFilterTextChanged( const FText& InFilterText )
{
...
void SBangoScriptClassViewer::OnFilterTextCommitted(const FText& InText, ETextCommit::Type CommitInfo)
{
	if (CommitInfo == ETextCommit::OnEnter)
	{
		if (InitOptions.Mode == EClassViewerMode::ClassPicker)
		{
			TArray< TSharedPtr< FBangoScriptClassViewerNode > > SelectedList = ClassList->GetSelectedItems();
			TSharedPtr< FBangoScriptClassViewerNode > FirstSelected;
...
bool SBangoScriptClassViewer::Menu_CanExecute() const
{
	return true;
}

void SBangoScriptClassViewer::MenuActorsOnly_Execute()
{
...
bool SBangoScriptClassViewer::MenuActorsOnly_IsChecked() const
{
	return InitOptions.bIsActorsOnly;
}

void SBangoScriptClassViewer::MenuPlaceableOnly_Execute()
{
...
bool SBangoScriptClassViewer::MenuPlaceableOnly_IsChecked() const
{
	return InitOptions.bIsPlaceableOnly;
}

void SBangoScriptClassViewer::MenuBlueprintBasesOnly_Execute()
{
...
bool SBangoScriptClassViewer::MenuBlueprintBasesOnly_IsChecked() const
{
	return InitOptions.bIsBlueprintBaseOnly;
}

TSharedRef<SWidget> SBangoScriptClassViewer::FillFilterEntries()
{
	FMenuBuilder MenuBuilder(true, nullptr);
	MenuBuilder.BeginSection("ClassViewerFilterEntries");
	{
		MenuBuilder.AddMenuEntry( LOCTEXT("ActorsOnly", "Actors Only"), LOCTEXT( "ActorsOnly_Tooltip", "Filter the Class Viewer to show only actors" ), FSlateIcon(), FUIAction(FExecuteAction::CreateRaw(this, &SBangoScriptClassViewer::MenuActorsOnly_Execute), FCanExecuteAction::CreateRaw(this, &SBangoScriptClassViewer::Menu_CanExecute), FIsActionChecked::CreateRaw(this, &SBangoScriptClassViewer::MenuActorsOnly_IsChecked)), NAME_None, EUserInterfaceActionType::Check );
		MenuBuilder.AddMenuEntry( LOCTEXT("PlaceableOnly", "Placeable Only"), LOCTEXT( "PlaceableOnly_Tooltip", "Filter the Class Viewer to show only placeable actors." ), FSlateIcon(), FUIAction(FExecuteAction::CreateRaw(this, &SBangoScriptClassViewer::MenuPlaceableOnly_Execute), FCanExecuteAction::CreateRaw(this, &SBangoScriptClassViewer::Menu_CanExecute), FIsActionChecked::CreateRaw(this, &SBangoScriptClassViewer::MenuPlaceableOnly_IsChecked)), NAME_None, EUserInterfaceActionType::Check );
	}
	MenuBuilder.EndSection();

	MenuBuilder.BeginSection("ClassViewerFilterEntries2");
	{
		MenuBuilder.AddMenuEntry( LOCTEXT("BlueprintsOnly", "Blueprint Class Bases Only"), LOCTEXT( "BlueprinsOnly_Tooltip", "Filter the Class Viewer to show only base blueprint classes." ), FSlateIcon(), FUIAction(FExecuteAction::CreateRaw(this, &SBangoScriptClassViewer::MenuBlueprintBasesOnly_Execute), FCanExecuteAction::CreateRaw(this, &SBangoScriptClassViewer::Menu_CanExecute), FIsActionChecked::CreateRaw(this, &SBangoScriptClassViewer::MenuBlueprintBasesOnly_IsChecked)), NAME_None, EUserInterfaceActionType::Check );
	}
	MenuBuilder.EndSection();
...
void SBangoScriptClassViewer::SetAllExpansionStates(bool bInExpansionState)
{
...
void SBangoScriptClassViewer::SetAllExpansionStates_Helper(TSharedPtr< FBangoScriptClassViewerNode > InNode, bool bInExpansionState)
{
...
void SBangoScriptClassViewer::ToggleExpansionState_Helper(TSharedPtr< FBangoScriptClassViewerNode > InNode)
{
	bool bExpanded = ClassTree->IsItemExpanded( InNode );
	ClassTree->SetItemExpansion(InNode, !bExpanded);
}

bool SBangoScriptClassViewer::ExpandFilteredInNodes(TSharedPtr<FBangoScriptClassViewerNode> InNode)
{
...
void SBangoScriptClassViewer::MapExpansionStatesInTree( TSharedPtr<FBangoScriptClassViewerNode> InItem )
{
...
void SBangoScriptClassViewer::SetExpansionStatesInTree( TSharedPtr<FBangoScriptClassViewerNode> InItem )
{
...
int32 SBangoScriptClassViewer::CountTreeItems(FBangoScriptClassViewerNode* Node)
{
	if (Node == nullptr)
	{
		return 0;
	...
	int32 Count = 1;
	TArray<TSharedPtr<FBangoScriptClassViewerNode>>& ChildArray = Node->GetChildrenList();
...
void SBangoScriptClassViewer::Populate()
{
	TArray<FTopLevelAssetPath> PreviousSelection;
	{
		TArray<TSharedPtr<FBangoScriptClassViewerNode>> SelectedItems = GetSelectedItems();
		if (SelectedItems.Num() > 0)
		{
			for (TSharedPtr<FBangoScriptClassViewerNode>& Node : SelectedItems)
			{
				if (Node.IsValid())
				{
	...
	if (!IsShowingInternalClasses())
	{
		...
		for (int i = 0; i < InternalClassNames.Num(); i++)
		{
			...
			const TSharedPtr<FBangoScriptClassViewerNode> ClassNode = ClassViewer::Helpers::ClassHierarchy->FindNodeByGeneratedClassPath(ClassViewer::Helpers::ClassHierarchy->GetObjectRootNode(), PackageClassName);
	...
	if(InitOptions.DisplayMode == EClassViewerDisplayMode::TreeView)
	{
		...
		TSharedPtr<FBangoScriptClassViewerNode> RootNode;
		...
		TSharedPtr<FBangoScriptClassViewerNode> ClassNode;
		TSharedPtr<FBangoScriptClassViewerNode> ExpandNode;
		...
		else if (InitOptions.InitiallySelectedClass)
		{
			...
			{
				for (const TSharedPtr<FBangoScriptClassViewerNode>& ChildClassNode : ClassNode->GetChildrenList())
				{
	...
	else
	{
		...
		{
			RootTreeItems.Sort([this](const TSharedPtr<FBangoScriptClassViewerNode>& A, const TSharedPtr<FBangoScriptClassViewerNode>& B)
			{
		...
		else
		{
			...
			RootTreeItems.Sort(FBangoScriptClassViewerNodeNameLess(InitOptions.NameTypeToDisplay));
		}
		...
		{
			if(TSharedPtr<FBangoScriptClassViewerNode>* ClassNode = RootTreeItems.FindByPredicate([ClassPathNameToSelect](const TSharedPtr< FBangoScriptClassViewerNode > InClassNode) { return InClassNode->Class.IsValid() && (InClassNode->Class->GetPathName() == ClassPathNameToSelect); }))
			{
...
FReply SBangoScriptClassViewer::OnKeyDown( const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent )
{
...
FReply SBangoScriptClassViewer::OnFocusReceived( const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent )
{
...
bool SBangoScriptClassViewer::SupportsKeyboardFocus() const 
{
	return true;
}

void SBangoScriptClassViewer::RequestPopulateClassHierarchy()
{
	ClassViewer::Helpers::RequestPopulateClassHierarchy();
}

void SBangoScriptClassViewer::DestroyClassHierarchy()
{
	ClassViewer::Helpers::DestroyClassHierachy();
}

TSharedPtr<FBangoScriptClassViewerNode> SBangoScriptClassViewer::CreateNoneOption()
{
	TSharedPtr<FBangoScriptClassViewerNode> NoneItem = MakeShared<FBangoScriptClassViewerNode>("None", "None");
...
void SBangoScriptClassViewer::Refresh()
{
	bNeedsRefresh = true;
}

void SBangoScriptClassViewer::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{	
...
bool SBangoScriptClassViewer::IsClassAllowed(const UClass* InClass) const
{
	return ClassFilter->IsClassAllowed(InitOptions, InClass, ClassFilter->FilterFunctions);
}

void SBangoScriptClassViewer::HandleSettingChanged(FName PropertyName)
{
...
void SBangoScriptClassViewer::ToggleShowInternalClasses()
{
...
bool SBangoScriptClassViewer::IsToggleShowInternalClassesAllowed() const
{
	return InitOptions.bAllowViewOptions;
}

bool SBangoScriptClassViewer::IsShowingInternalClasses() const
{
	if (!InitOptions.bAllowViewOptions)
	{
		return true;
...
void SBangoScriptClassViewer::ToggleCustomFilterOption(TSharedRef<FClassViewerFilterOption> FilterOption)
{
...
bool SBangoScriptClassViewer::IsCustomFilterOptionEnabled(TSharedRef<FClassViewerFilterOption> FilterOption) const
{
