...
class SSearchBox;
class FBangoScriptsScriptContainerClassViewerFilter;


// Based on SClassViewer. Thanks Epic for making everything private instead of protected, it was real helpful!
class SBangoScriptClassViewer : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( SBangoScriptClassViewer )
	{
	}
	...
	virtual ~SBangoScriptClassViewer();
...
private:
	 ...
	 *
	 */
	void OnGetChildrenForClassViewerTree( TSharedPtr<FBangoScriptClassViewerNode> InParent, TArray< TSharedPtr< FBangoScriptClassViewerNode > >& OutChildren );

	/** Creates the row widget when called by Slate when an item appears on the tree. */
	TSharedRef< ITableRow > OnGenerateRowForClassViewer( TSharedPtr<FBangoScriptClassViewerNode> Item, const TSharedRef< STableViewBase >& OwnerTable );
	...
	void OnClassViewerSelectionChanged( TSharedPtr<FBangoScriptClassViewerNode> Item, ESelectInfo::Type SelectInfo );

	/** Called by Slate when an item is expanded/collapsed from the tree/list. */
	void OnClassViewerExpansionChanged(TSharedPtr<FBangoScriptClassViewerNode> Item, bool bExpanded);
	 ...
	 *	@param	bInExpansionState			The expansion state to set the tree to.
	 */
	void SetAllExpansionStates_Helper(TSharedPtr< FBangoScriptClassViewerNode > InNode, bool bInExpansionState);
	 ...
	 */
	void ToggleExpansionState_Helper(TSharedPtr< FBangoScriptClassViewerNode > InNode);
	...
	*/
	bool ExpandFilteredInNodes(TSharedPtr<FBangoScriptClassViewerNode> InNode);
	 ...
	 */
	void MapExpansionStatesInTree( TSharedPtr<FBangoScriptClassViewerNode> InItem );
	 ...
	 */
	void SetExpansionStatesInTree( TSharedPtr<FBangoScriptClassViewerNode> InItem );

	/** Populates the tree with items based on the current filter. */
	void Populate();

	/** Creates a "None" option for the tree/list. */
	TSharedPtr<FBangoScriptClassViewerNode> CreateNoneOption();

	/** Returns an array of the currently selected ClassViewerNodes */
	const TArray< TSharedPtr< FBangoScriptClassViewerNode > > GetSelectedItems() const;
	...
	int32 CountTreeItems(FBangoScriptClassViewerNode* Node);
...
private:
	...
	TSharedPtr<FBangoScriptsScriptContainerClassViewerFilter> ClassFilter;

	/** The items to be displayed in the tree. */
	TArray< TSharedPtr< FBangoScriptClassViewerNode > > RootTreeItems;

	/** Holds the Slate Tree widget which holds the classes for the Class Viewer. */
	TSharedPtr<STreeView<TSharedPtr< FBangoScriptClassViewerNode > >> ClassTree;

	/** Holds the Slate List widget which holds the classes for the Class Viewer. */
	TSharedPtr<SListView<TSharedPtr< FBangoScriptClassViewerNode > >> ClassList;
