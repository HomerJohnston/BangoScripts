#include "BangoScripts/Uncooked/K2Nodes/K2Node_BangoSleep.h"
...
void UK2Node_BangoSleep::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	static const TArray<FName> PinControlVars
	{
		FName("bInfiniteDuration"), 
		FName("bEnableSkipExecPin"), 
		FName("bEnableCancelExecPin"),
		FName("bEnableSkipConditionPin"), 
		FName("bEnableCancelConditionPin"), 
		FName("bEnablePauseConditionPin"),
	};
	
	if (PinControlVars.Contains(PropertyChangedEvent.MemberProperty->GetName()))
	{
		ReconstructNode();
	}
}
...
{	
	...
	if (IsCancelConditionPinEnabled())
	{
...
void UK2Node_BangoSleep::ExpandNode(class FKismetCompilerContext& Compiler, UEdGraph* SourceGraph)
{
	...
	if (bUseSkip)
	{
		...
		{
			Builder.MoveExternalConnection(Node_SkipExecTriggered.Variable, Node_SkipConditionsOR.A);	
		}
		if (Node_This.SkipCondition)
		{
			Builder.MoveExternalConnection(Node_This.SkipCondition, Node_SkipConditionsOR.B);
		}
	}
	if (bUsePause)
	{
